...
天生施法者: 龙形态的伊汐利亚斯相当于一位14级术士，他的施法能力是魅力(DC20, +12)，他在施法时不需要材料，一天可以释放魅力调整值数量的法术。
...
吞噬魔能(1/日): 损失半数生命值后强制使用，伊汐利亚斯汲取大量的真空能量给自身充能，充能过程中获得全部伤害抗性与100点临时生命值，充能耗时1回合，充能时无法进行其他动作，在充能期间收到100点伤害，将会打断充能，充能完成后失去所有的临时生命，并获得`魔力满盈`状态。
...
强大的真空能量充斥在伊汐利亚斯身体之中，身体里满盈的能量让他可以无视各种法术(除力场以外的伤害的抗性且三环以下法术对其无效)，同时纯粹的魔法伤害(力场)会给他恢复生命值，但因为伊汐利亚斯对于真空能量浅薄的理解，在战斗中他会发现自己逐渐无法控制这股能量(除了满盈后的第一个回合，每一个回合开始前都要进行一次无加值的d20的检定，第一次检定难度为10，通过检定后，下次检定难度会+5)。

检定失败后，伊汐利亚斯对这股能量失去控制，会受到(5d10 + 50 + 魔力满溢恢复的生命值)的力场伤害，该伤害无法被减免，如果该伤害会造成伊汐利亚斯死亡，那就保留一点生命值。

满盈施法: 体内澎湃的能量使得伊汐利亚斯可以随意释放法术，使用魔能吐息后，进行3次施法。

魔能吐息(1/回合): 每个回合开始时强制执行一次吐息，伊汐利亚斯将体内汹涌的魔能倾泻而出，因为无法控制，在吐息时进行一次d6，决定这次吐息的伤害属性。(1个动作)。
