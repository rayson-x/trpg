...
- 当你使用5环或者更高环位的法术位施展该法术时，每比4环高出一环的法术位可以使该法术的伤害增加1d10。
...
猛击: 近战攻击，命中+9，范围为5尺，单一目标，伤害(3d8+6)。消耗一个动作

缓速(充能5-6): 魔像可以选择一个或多个在其10英尺范围内且它能够看见的生物作为目标。每个目标都必须进行一次DC 17的感知豁免以抵抗这个魔法。如果豁免失败，目标将无法使用反应，其速度减半，并且在其回合中不能进行超过一次攻击。此外，目标在其回合中只能进行一个动作或一个附赠动作，而不能在同一回合内使用两者。这些效果持续最多1分钟。目标可以在每次其回合结束时重复进行豁免检定，成功则结束这些效果对其的影响。消耗一个动作
...
强效施法: 投掷法术伤害骰时，可以重骰等于魅力调整值的骰子数量，必须使用新的结果(1/回合)。

瞬发施法: 可以用附赠动作释放一次法术。

升阶施法: 释放一个需要豁免检定的法术时，可以指定一个目标在对抗法术的第一次豁免中承受劣势(1/回合)。
...
|  力 量  |  敏 捷 |  体 质  |  智 力  | 感 知  |  魅 力 |
---------|--------|--------|--------|--------|-------|
| 25(+7) | 12(+1) | 23(+6) | 19(+4) | 16(+3) | 20(+5)|

---

豁免: 敏捷+7，体质+13，感知+9，魅力+10

技能: 欺瞒+11，察觉+10，感知+17，说服+11，隐匿+8

伤害抗性: 非精金武器且非魔法攻击所造成的钝击，穿刺，劈砍
...
天生施法者: 伊汐利亚斯相当于一位14级术士，他的施法能力是魅力(DC20 +12,进行法术攻击)，他在施法时不需要材料。
...
施法: 从他的术士法表中选一个法术进行释放，一回合只能使用一次。(1个动作)

啃咬: 近战武器攻击,范围为10尺,单一目标.若命中(2d10+6)穿刺伤害加(2d6)毒素伤害。(1个动作)

爪抓: 近战武器攻击,范围为5尺,单一目标.若命中(2d6 + 6)劈砍伤害。(1个动作)

尾击: 近战武器攻击,范围为15尺,单一目标.若命中(2d8 + 6)钝击伤害。(1个动作)

恐吓威仪: 每个距离伊汐利亚斯120尺内，由伊汐利亚斯选择的且意识到其存在的生物都必须成功通过一次(DC18)的感知豁免，否则将陷入恐惧状态，持续1分钟。一个生物可以在它的每个回合结束时重复这个豁免检定，若检定成功则结束这个效果对它的影响。若一个生物成功通过该豁免检定或结束这个效果对它的影响，则该生物将在接下来的24小时内免疫恐吓威仪。(1个动作)

剧毒吐息(充能5–6): 吐出一道60尺锥形的剧毒气体。每个在吐息范围内的生物都必须进行一次(DC21)的体质豁免，豁免失败则受到(8d8)毒素伤害,豁免成功则只受到一半的伤害。(2个动作)

魔化龙鳞: 伊汐利亚斯用真空能量给身上的鳞片充能，充能后的龙鳞将低于4点的伤害视为0，持续一个回合。(1个动作)

吞噬魔能(一场战斗一次): 损失半数生命值后强制使用，伊汐利亚斯汲取大量的真空能量给自身充能，充能过程中获得全部伤害抗性与100点临时生命值，充能耗时1回合，充能时无法进行其他动作，在充能期间收到100点伤害，将会打断充能，充能完成后失去所有的临时生命，并获得`魔力满盈`状态。(3个动作)

### 魔力满盈

---

强大的真空能量充斥在伊汐利亚斯身体之中，身体里满盈的能量让他可以无视各种法术(自选四种法术伤害的抗性或三环以下法术对其无效)，但因为伊汐利亚斯对于真空能量浅薄的理解，在战斗中他会发现自己逐渐无法控制这股能量(除了满盈后的第一个回合，每一个回合开始前都要进行一次无加值的d20的检定，第一次检定难度为10，通过检定后，下次检定难度会+5)。

检定失败后，伊汐利亚斯对这股能量失去控制，会受到(6d10 + 40)的力场伤害，该伤害无法被减免，如果该伤害会造成伊汐利亚斯死亡，那就保留一点生命值。

施法: 体内澎湃的能量使得伊汐利亚斯可以随意释放法术，此时施法不受任何限制。(一个动作)

魔能吐息: 伊汐利亚斯将体内汹涌的魔能倾泻而出，因为无法控制，在吐息时进行一次d6，决定这次吐息的伤害属性。(2个动作)。
...
- 魔能类型
  ...
  - 6: 吐出一道耀眼的光束，在120尺长，10尺宽的范围内造成8d12的光耀伤害，需要通过DC21的体质豁免，豁免失败会进入目盲状态，豁免成功的生物只收到一半的伤害。
...
满盈施法: 魔能满盈状态下，可以施展法术(1环:消耗1动作, 2环:消耗2动作, 3环:消耗3动作)。

振翅攻击(消耗2动作): 伊汐利亚斯拍打它的翅膀。每个距离伊汐利亚斯10尺内的生物都必须成功通过一次(DC22)的敏捷豁免，否则受到(2d6+6)钝击伤害，并被击倒伏地。伊汐利亚斯接着可以飞行最多等同它飞行速度一半的距离。
...
- 智慧湮灭(7)
  - 伊汐利亚斯可以集中精神与一个距离120尺内能看到的生物建立心灵链接，除非有心灵屏障阻挡。知识吞噬者伊汐利亚斯将用自己在无尽书库阅读了400年的知识灌输给目标，被选中的目标必须通过一次(DC24)的智力豁免，若豁免失败，将受到强烈的心灵冲击(10d10心灵)，并陷入无尽的信息洪流之中(昏迷状态)。若豁免通过将受到一半的伤害并不会陷入昏迷，并且智力提高至21，持续一小时。同时可以过一个(DC15)历史，DM可以酌情告知一些历史设定/敌人属性/经历。
...
- 自然之怒(5)(专注1分钟)
  - 你呼唤自然精魂以对抗你的敌人。选择射程内你所能看见的一点。该精魂将使以该点为中心的60尺边长立方体内的树木、岩石和草丛活化，直到此法术结束。
  - 草地和矮树丛：立方体区域内任何被草地或矮树丛覆盖的地表对你的敌人而言视作困难地形。
  - 树木：在你的每个回合开始时，每个距离立方体区域内任何树木10尺内的敌人都必须成功通过一次敏捷豁免，否则受到来自枝杈鞭打的4d6劈砍伤害。
  - 树根与藤蔓：在你的每个回合结束时，一个你所选择的、在立方体区域内地表上的生物必须成功通过一次力量豁免，否则被束缚直到此法术结束。一个被束缚的生物可以使用一个动作进行一次对抗你法术豁免DC的力量（运动）检定，若检定成功，则此效果对它的影响结束。
  - 岩石：在你的回合中，你可以作为一个附赠动作使立方体区域内的一个松动岩石投向立方体区域内一个你所能看见的生物。对目标进行一次远程法术攻击。若命中，目标将受到3d8非魔法钝击伤害，且它必须成功通过一次力量豁免，否则被击倒伏地。
